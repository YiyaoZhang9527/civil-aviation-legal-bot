...
# 标题清理：去掉外层《》包装、PDF抬头残留（"已于YYYY年MM月"等）
_TITLE_BOOKMARK_RE = re.compile(r"^[#\s]*《(.+?)》\s*(?:已于)?\s*\d{4}\s*年\s*\d{1,2}\s*月\s*$")
_TITLE_BOOKMARK_PLAIN_RE = re.compile(r"^[#\s]*《(.+?)》\s*$")


def _clean_law_title(candidate: str) -> str:
    """剥离 Markdown 标题符号、外层《》包装、PDF抬头残留后缀。"""
    s = candidate.strip().lstrip("#").strip()
    m = _TITLE_BOOKMARK_RE.match(s)
    if m:
        return m.group(1).strip()
    m = _TITLE_BOOKMARK_PLAIN_RE.match(s)
    if m:
        return m.group(1).strip()
    return s
...
def _clean_line(line: str) -> str:
    ...
    # 去除 markdown 表格符号：去掉行首尾的 | 和连续空格
    # 例："|  | 第一条 内容 |  |  |" → "第一条 内容"
    if getattr(config, "PARSER_STRIP_MD_TABLE", True):
...
def parse_law_text(source_path: Path) -> LawDocument:
    ...
    for candidate in non_empty[:10]:
        if candidate and not re.fullmatch(r"[=—\-_*•·\s]+", candidate) and not _SKIP_TITLE_RE.match(candidate):
            title = _clean_law_title(candidate)
            break
