...
SM_FORCE_DEMOTE_ON_FAIL = True            # 方案A: SM 失败时强制 source 降级为"一般建议"（旧：仅加⚠️待核实）
CLAIM_LEVEL_CITATION = False              # Claim→Evidence 校验方向（消融实验：新方向太激进，旧方向evidence→claim更均衡）
HARD_REFUSAL_ON_EMPTY_EVIDENCE = True     # 方案A: 证据为空时硬拒答（避免 LLM 凭通用法律知识编造）
...
TREE_EARLY_ARTICLE_PENALTY = 0.4            # 位置降权系数：前 N 条（总则候选）额外降权（消融 2026-06-06: 0.4 在 5 题探针上 100% LeMAJ）
...
SYNTHESIS_EVIDENCE_TRUNCATE = 3000        # 传给 LLM 的单条证据截断字符数
...
CRAG_ENABLED = True                        # 方案B: CRAG 检索质量补救（验证有效，启用）
CRAG_MAX_RETRIES = 1                       # 方案B: 质量不足时补救检索次数
CRAG_QUALITY_THRESHOLD = "partial"         # 方案B: 触发补救的最低质量(sufficient/partial/insufficient)
